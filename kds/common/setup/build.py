...
from kds.config.base import BASEDIR, DOCDIR
from kds.common.utils import get_path, find_files, list_files, list_items
...
class build_css(Command):
    ...
    def run(self):
        ...
        CSSFULL_DIR = get_path([BASEDIR, 'kds', 'data', 'static', 'css',
                                'full'])
        CSSMIN_DIR = get_path([BASEDIR, 'kds', 'data', 'static', 'css',
                               'min'])
...
class build_man(Command):
    ...
    def run(self):
        ...
        pub.publish(argv=[u'%s' % get_path([DOCDIR, 'man', 'kds.rst']),
                          u'%s' % get_path([DOCDIR, 'man', 'kds.1'])])
