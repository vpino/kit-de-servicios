...
def get_data_files(path=None, patterns=None, exclude_files=None):
    ...
    a directory. This function helps the KDSkds Maintainer to define a list of
    files to be installed in a certain system directory.

    For example, generated documentation (everything under
    ``kds/data/docs/html`` after executing ``make build_sphinx``) should be
    put in ``/usr/share/doc/kds/``. The maintainer should add a pattern like
    the following so that everything from ``kds/data/docs/html`` gets copied
    to ``/usr/share/doc/kds/`` when the package is installed
...
def get_setup_data(basedir):
    ...
    .. versionadded:: 0.1
    """
    from kds.config.base import (NAME, VERSION, URL, AUTHOR, AUTHOR_EMAIL,
                                    DESCRIPTION, LICENSE, DOCDIR)
    from kds.config.pkg import (classifiers, long_description,
                                   install_requires, dependency_links,
                                   exclude_packages, platforms, keywords)
    from kds.common.version import get_version
    from kds.common.setup.build import (build_man, build_css, build_js,
                                           build_sphinx, compile_catalog,
                                           build)
    from kds.common.setup.clean import (clean_mo, clean_sphinx, clean_js,
                                           clean_css, clean_man, clean_img,
                                           clean_dist, clean_pyc, clean)
    from kds.common.setup.install import install_data, build_py
    from kds.common.setup.maint import (extract_messages, init_catalog,
                                           update_catalog)
    from kds.common.setup.report import report_setup_data
    ...
    return {
        ...
        'test_suite': 'kds.common.testsuite.runtests',
        'zip_safe': False,
        'cmdclass': {
            'clean': clean,
        ...
        'message_extractors': {
            'kds': [
                ('**.html',
                 'kds.common.setup.message_extractors:django', ''),
                ('**.py', 'python', ''),
            ],
        },
        'command_options': {
            ...
            'update_catalog': {
                ...
                'input_file': ('setup.py', 'kds/data/i18n/pot/django.pot'),
                'output_dir': ('setup.py', 'kds/data/i18n'),
                'ignore_obsolete': ('setup.py', True),
                'previous': ('setup.py', False),
            },
            'compile_catalog': {
                'domain': ('setup.py', 'django'),
                'directory': ('setup.py', 'kds/data/i18n'),
            ...
            'extract_messages': {
                'copyright_holder': ('setup.py', 'Desarrolladores de Kds'),
                'msgid_bugs_address':
                ('setup.py',
                    'desarrolladores@canaima.softwarelibre.gob.ve'),
                'output_file': ('setup.py', 'kds/data/i18n/pot/django.pot'),
